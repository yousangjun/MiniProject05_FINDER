...
const Post_jobs_com = () => {
    ...
    const handleGetCompany = async () => {
        try {
            const response = await getComToUserNo(userInfo.userNo);
            setCompany(response.data.comNo);
        } catch (error) {
    ...
    const handleSubmit = async (event) => {
        ...
        files.forEach((file, index) => {
            formData.append('files', file); // files 배열을 순회하며 각각의 파일을 추가
        });
